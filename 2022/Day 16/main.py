...
total_score =0
best_score = 0
def dfs(small_G, current_node, time, total_score):
    global best_score
    ...
    for neighbor in small_G.neighbors(current_node):
        if small_G.nodes[neighbor]['visited'] == 1:
            ...
            dfs(small_G, neighbor, time_, total_score+(small_G.nodes[neighbor]['value']*time_))
...
dfs(small_G, current_node, time, total_score)
