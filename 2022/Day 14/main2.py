...
with open('./Day 14/test_input.txt') as f:
    lines = f.readlines()
...
#crate matrix
x_max = max([max([pair[0] for pair in line]) for line in lines])+500
y_max = max([max([pair[1] for pair in line]) for line in lines])+2
...
def show_matrix(matrix):
    display_matrix = np.array(matrix)[:,494:510]
...
while True:
    ...
    # if sand is still at the top, stop
    if sand_pos[1] == 0:
        break
