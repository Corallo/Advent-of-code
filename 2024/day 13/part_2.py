...
def solve_problem(problem_data):
    ...
    push_b = Int('b')
    constraints = []
    constraints.append(push_a * problem_data[0] + push_b * problem_data[2] == 10000000000000+problem_data[4])
    constraints.append(push_a * problem_data[1] + push_b * problem_data[3] == 10000000000000+problem_data[5])
    s = Solver()
    s.add(constraints)
    if s.check() == sat:
        model = s.model()
...
submit(pushes, part="b", day=DAY, year=2024, session=session_id)